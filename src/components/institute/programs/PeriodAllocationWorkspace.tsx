...
const SubProgramSwitcher: React.FC<{
  subPrograms: NonNullable<InstituteProgram['subPrograms']>;
  activeId: string;
  slices: ScheduleConfig['subProgramSlices'];
  currentFlatTotal: number;
  onSwitch: (id: string) => void;
}> = ({ subPrograms, activeId, slices, currentFlatTotal, onSwitch }) => {
  const [pulseId, setPulseId] = React.useState<string | null>(null);

  const totalFor = (id: string) => {
    if (id === activeId) return currentFlatTotal;
    const slice = slices?.[id];
    if (!slice) return 0;
    return Object.values(slice.trackTargetPeriods ?? {}).reduce(
  ...
  const active = subPrograms.find((s) => s.id === activeId) ?? subPrograms[0];

  return (
    <Card className="border-indigo-200 shadow-sm bg-gradient-to-r from-indigo-50/70 via-white to-blue-50/40">
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center gap-2">
          <Layers className="h-4 w-4 text-indigo-600" />
          <span className="text-[11px] uppercase tracking-wider font-bold text-indigo-700">Programs</span>
          <span className="text-[11px] text-slate-500">
            · each program manages its own subject targets, tracks &amp; weekly timetable within the shared section
          </span>
        </div>
        <div className="flex flex-wrap gap-2">
          {subPrograms.map((sp) => {
            const isActive = sp.id === active.id;
            const isPulsing = pulseId === sp.id;
            const periods = totalFor(sp.id);
            return (
              <button
                key={sp.id}
                onClick={() => handle(sp.id)}
                title={`${sp.name} · ${periods} periods`}
                className={cn(
              ...
              >
                ...
                <div className="flex flex-col leading-tight">
                  ...
                  <span className={cn('text-[10px] truncate max-w-[160px]', isActive ? 'text-indigo-100' : 'text-slate-500')}>
                    {sp.name}
                  </span>
                </div>
                <span
        ...
        <p className="text-[11px] text-slate-600">
          Editing <strong className="text-slate-900">{active.code}</strong> — switching loads that program's
          own subject targets, tracks &amp; weekly timetable. Working days, periods/day &amp; faculty pool stay shared.
        </p>
      </CardContent>
    </Card>
  );
